...
export default function InfoCardLeft({ title = "", subtitle = "Expert Help At Every IB Milestone", description = "", infoRow = [], buttons = [], altText = "IB Curriculum" }) {
    const handleAction = (action) => (e) => {
        if (action === "scroll") {
            e.preventDefault();
            document.querySelector(".right-form")?.scrollIntoView({ behavior: "smooth", block: "start" });
        } else if (action === "shake") {
            e.preventDefault();
            document.dispatchEvent(new CustomEvent("ignite:formShake"));
        } else if (action === "shakeOrScroll") {
            e.preventDefault();
    ...
    return (
        <div className="col-lg-8 d-flex flex-column justify-content-center pe-lg-4 p-4 left-content v100">
            <h1 className="fw-bold text-white text-uppercase mb-3  hero-h1">
                {title && title.split(/\s*<br\s*\/?\s*>\s*/i).map((part, i, arr) => (
            ...
            <div className="d-flex gap-3 btnwraper ">
                {buttons.map((btn, idx) => {
                    ...
                    if (btn.action) {
                        return (
                            <button
                                key={`btn-${idx}-${btn.text}`}
                                type="button"
                                onClick={handleAction(btn.action)}
                                style={{ textDecoration: "none", border: "none", background: "transparent", padding: 0 }}
                            >
                                <div className="btn cust-text btng fw-bold text-uppercase d-flex justify-content-between align-items-center shadow left-btn" style={innerStyle}>
                                    {inner}
                                </div>
                            </button>
                        );
                    ...
                    return (
                        <a key={`btn-${idx}-${btn.text}`} href={btn.href} style={{ textDecoration: "none" }}>
                            <button className="btn cust-text btng fw-bold text-uppercase d-flex justify-content-between align-items-center shadow left-btn" style={innerStyle}>
                                {inner}
                            </button>
                        </a>
                    );
        ...
        /* Top-anchor content on desktop so the start position is identical on
           every page regardless of description length — long text flows down
           instead of re-centering the whole block. */
        @media (min-width: 992px) {
          .left-content {
