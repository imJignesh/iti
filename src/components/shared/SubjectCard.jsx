...
export default function SubjectCard({ config = {} }) {
  const {
    ...
    >
      <div className="text-center mb-md-5 mb-4 fade-in-section"
        ...
        <div className="testHeadings">
          ...
          >
            <h2 className="SubHeading testSubheading">{subHeading}
            </h2>
          </div>
          {titleTag === "h3" ? (
          ...
          <div className="subjects-card1-right">
            <div className="subjects-card1-bubbles-grid">
              {subjectRows.map((row, rowIdx) => (
                ...
                >
                  {row.map((subj) => {
                    const link = subjectLinks && subjectLinks[subj];
                    if (link) {
                      return bubbleTag === "h3" ? (
                        <a key={subj} href={link}>
                          <h3 className="subjects-card1-bubble">{subj}</h3>
                        </a>
                      ) : (
                        <a key={subj} href={link}>
                          <div className="subjects-card1-bubble">{subj}</div>
                        </a>
                      );
                    }
                    return bubbleTag === "h3" ? (
      ...
      <style jsx>{`
        ...
        /* .container's width/max-width/margin/padding come from the
           global .container class (src/styles/globals.css) which uses
           !important and always wins; only the 767px padding override
           below actually differs from the global rule. */
