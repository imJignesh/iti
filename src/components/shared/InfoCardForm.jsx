...
export default function InfoCardForm({ formConfig }) {
    ...
    const selectFields = formConfig?.selectFields || [];
    const messageConfig = formConfig?.message || { name: "message", placeholder: "DROP A MESSAGE", delay: "0.85s" };

    const initialFormData = { type: formConfig?.type || "Organic_Subject" };
    fields.forEach(f => { initialFormData[f.name] = ""; });
    selectFields.forEach(f => { initialFormData[f.name] = ""; });
    initialFormData[messageConfig.name] = "";
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch('/api/submit-form', {
                ...
                body: JSON.stringify({ ...formData, pageinfo: pageInfo }),
            });
        ...
        <div className="col-lg-4 form-bg mt-4 mt-lg-0 d-flex align-items-center position-relative right-form" style={{ animationDelay: "0.6s" }}>
            <div className={`w-100 text-white form-container ${isShaking ? 'shake-animation' : ''}`}>
                <form onSubmit={handleSubmit}>
                    ...
                    {submissionStatus === 'success' && <div className="alert alert-success text-center">Thank you! Your submission was successful.</div>}
                    {submissionStatus === 'error' && <div className="alert alert-danger text-center">Submission failed. Please try again.</div>}

                    {/* Render Text Input Fields */}
                    {fields.map((field, idx) => {
                        // Skip if this is school field (it's handled with grade field for Organic_Subject types)
                        if (field.name === 'school' && formConfig?.type === 'Organic_Subject') return null;

                        if (field.type === 'email' && field.withPhone) {
                            return (
                                <div key={field.name} className="row g-2 mb-3" style={{ animationDelay: field.delay }}>
                                    <div className="col-6">
                                        <input
                                            type={field.type}
                                            name={field.name}
                        ...
                        // Grade field - render with school in same row (col-4 and col-8) ONLY for Organic_Subject pages
                        if (field.name === 'grade' && formConfig?.type === 'Organic_Subject') {
                            const schoolField = fields.find(f => f.name === 'school');
                            return (
                                <div key="grade-school" className="row g-2 mb-3" style={{ animationDelay: field.delay }}>
                                    <div className="col-4">
                                        <input
                                            type={field.type}
                                            name={field.name}
                                    ...
                                    <div className="col-8">
                                        <input
                                            ...
                                            onChange={handleChange}
                                            className="form-control bg-transparent text-white fw-semibold"
                                            placeholder={schoolField?.label || "SCHOOL"}
                                            style={inputStyle}
                                        />
                        ...
                        return (
                            <div key={field.name} className="mb-3" style={{ animationDelay: field.delay }}>
                                <input
                                    type={field.type}
                                    name={field.name}
                                    value={formData[field.name] || ""}
                    ...
                    {/* Message Textarea */}
                    <div className="mb-4" style={{ animationDelay: messageConfig.delay }}>
                        <textarea
                            name={messageConfig.name}
                            value={formData[messageConfig.name] || ""}
                            onChange={handleChange}
                            className="form-control bg-transparent text-white fw-semibold"
