...
export default function IgniteAchievementsSideBySide({ config }) {
  ...
  const {
    ...
    buttonLink = "/join-free-demo-class/",
    backgroundImage = "/assets/Rectangle125.webp",
              ...
              {stats.slice(0, 2).map((stat, i) => (
                <div key={i} className="col-6">
                  <div className="stat-card-mobile">
                    <div className={`stat-number ${stat.color}`}>{stat.number}</div>
                    <p className={`stat-title-mobile ${stat.color}`}>{stat.description || stat.title}</p>
                  </div>
                </div>
              ))}
            </div>
