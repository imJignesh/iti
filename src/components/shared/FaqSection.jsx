...
export default function FaqSection({
  ...
  description = "",
  faqData = [],
  backgroundImage = "/assets/faqbg.webp",
}) {
  ...
  const renderHighlighted = (text) =>
    text.split(/(<highlight>.*?<\/highlight>)/gi).map((part, i) => {
      const match = part.match(/^<highlight>(.*?)<\/highlight>$/i);
      return match ? (
        <span key={i} className="highlight">{match[1]}</span>
      ) : (
        part
      );
    });
  ...
  const renderTitle = (text) =>
    (text || "").split(/\s*<br\s*\/?\s*>\s*/i).map((line, i, arr) => (
      <span key={i}>
        ...
        <div className="row g-2 w-100 align-items-center">
          ...
          <div className={`col-lg-6 d-flex flex-column justify-content-center ${isMobile ? 'text-center' : ''}`}>
            ...
            <div className="testimonialHeader" style={{ marginBottom: isMobile ? "0px" : "20px" }}>
              <h2 className="SubHeading" style={{ fontSize: isHighDpi ? "28px" : isMobile ? "" : "28px" }}>{subHeading}
              </h2>
            </div>
