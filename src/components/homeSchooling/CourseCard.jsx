...
function CourseCard() {
  ...
  // 2. State to track if the device is considered mobile
  const [isMobile, setIsMobile] = useState(null); // Use null initially to prevent hydration mismatch

  // Function to determine screen size
  const checkIsMobile = () => {
    // Media query matching the CSS breakpoint (max-width: 768px)
    const mediaQuery = window.matchMedia('(max-width: 768px)');
    setIsMobile(mediaQuery.matches);
  };

  // 3. Effect to run only once after mount to check screen size
  useEffect(() => {
  ...
  // 4. Handle initial server render/hydration
  if (isMobile === null) {
    // Render nothing or a loading state until the client side has determined the screen size.
    // This is crucial for Next.js/React hydration to avoid mismatch warnings.
  ...
  return (
    <>
      ...
      {/* 7. Render Mobile Version only if mobile */}
      {isMobile && (
      ...
      <style jsx>{`
        ...
        /* Mobile Styles - Keep the specific styles for the mobile elements */
        @media (max-width: 768px) {
          /* .desktop-version { display: none; } and .mobile-version { display: block; } are removed */
